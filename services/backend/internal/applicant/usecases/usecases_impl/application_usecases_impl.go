...
func SubmitApplication(ctx context.Context, userId, universityId, cycle string) error {
	...
	if err := persistUnifiedSubmittedApplication(
		ctx,
		conn,
		userId,
		universityId,
		...
		log.Error("failed to submit application in unified database",
			zap.Error(err),
...
func persistUnifiedSubmittedApplication(
	ctx context.Context,
	conn middlewares.DB,
	...
	if err != nil {
		return fmt.Errorf("failed to begin application submit transaction: %s", err.Error())
	}
	defer func() {
		if err != nil {
			_ = tx.Rollback(ctx)
	...
	if err = tx.Commit(ctx); err != nil {
		return fmt.Errorf("failed to commit application submit transaction: %s", err.Error())
	}
