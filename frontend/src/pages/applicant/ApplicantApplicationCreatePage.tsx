...
import type { ApplicantApplicationListItem, ApplicantTestScore } from "../../types/domain";
...
function formatTestScoreLabel(item: ApplicantTestScore): string {
...
export function ApplicantApplicationCreatePage() {
  if (APPLICATIONS_DISABLED) {
    return <ApplicationsDisabledNotice />;
  }
  return <ApplicantApplicationCreateForm />;
}

function ApplicationsDisabledNotice() {
  return (
...
function ApplicantApplicationCreateForm() {
  ...
  useEffect(() => {
    ...
    const loadDraft = async () => {
      ...
      setDraftError(null);
      try {
        const applications = await fetchApplicantApplications();
        ...
        const application = await fetchApplicantApplication(universityId, cycleToLoad);
        if (!mounted) {
          return;
        }

        if (application.status === "draft") {
          setCycle(application.applicationCycle);
          const existingData =
          ...
          markDraftLoaded(application.applicationCycle);
  ...
  useEffect(() => {
    ...
    const handle = window.setTimeout(() => {
      void (async () => {
        ...
        const saved = await saveDraft({ advanceStep: false, silent: true });
        if (saved) {
  ...
  }, [draftHydrated, draftSaveState, draftSignature, duplicateApplication, hasFormContent, flowLoading, saveDraft]);
  ...
  useEffect(() => {
    const onBeforeUnload = (event: BeforeUnloadEvent) => {
      ...
      event.returnValue = "";
      void saveDraft({ advanceStep: false, silent: true });
    };
    window.addEventListener("beforeunload", onBeforeUnload);
    return () => {
      window.removeEventListener("beforeunload", onBeforeUnload);
    };
  }, [duplicateApplication, hasFormContent, saveDraft]);
  ...
  const uploadFilesForField = async (field: SchemaField, files: File[]) => {
    ...
    setUploadError(null);
    try {
      const uploadedFiles = await uploadApplicantApplicationFiles(universityId, flow.cycle, files, fieldKey(field));
  ...
  const importSelectedTestScores = async () => {
    ...
    try {
      ...
      const result = await importApplicantProfileTestScoresToApplication(universityId, flow.cycle, selectedTestScoreIds);
        ...
        <Card className="border-2 border-green-500/20">
          ...
          <CardContent className="space-y-4">
            ...
            <div className="flex flex-col gap-3 sm:flex-row">
              <Link to={routes.applicant.applications} className="flex-1">
