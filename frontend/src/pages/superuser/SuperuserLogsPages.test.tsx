...
import { SuperuserAuditLogsPage } from "./SuperuserAuditLogsPage";
import { SuperuserServiceLogsPage } from "./SuperuserServiceLogsPage";
import { fetchSuperuserAuditLogs, fetchSuperuserServiceLogs } from "../../services/admin/superuserService";

vi.mock("../../services/admin/superuserService", () => ({
  fetchSuperuserServiceLogs: vi.fn(),
  fetchSuperuserAuditLogs: vi.fn(),
}));

describe("superuser log pages", () => {
  beforeEach(() => {
    vi.mocked(fetchSuperuserServiceLogs).mockResolvedValue({
      items: [
        {
          id: "svc-1",
    ...
    vi.mocked(fetchSuperuserAuditLogs).mockResolvedValue({
      items: [
        {
          id: "aud-1",
          timestamp: "2026-03-24T12:00:00Z",
  ...
  it("renders structured service log fields without crashing", async () => {
    render(<SuperuserServiceLogsPage />);
  ...
  it("applies service log filters through the page fetch call", async () => {
    ...
    await waitFor(() => {
      expect(vi.mocked(fetchSuperuserServiceLogs)).toHaveBeenLastCalledWith(
