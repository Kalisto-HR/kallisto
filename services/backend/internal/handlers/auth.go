...
func NewAuthHandler(applicantDB, adminDB *pgxpool.Pool) *AuthHandler {
	return &AuthHandler{
		applicantDB: applicantDB,
		adminDB:     adminDB,
		auditLogger: observability.NewAuditLogger(adminDB, nil),
		sessionStore: authsession.NewStore(adminDB),
		throttle:     authsession.NewThrottle(adminDB),
	}
}
...
func (h *AuthHandler) SignUp(w http.ResponseWriter, r *http.Request) {
	...
	inUse, err := h.emailExistsAcrossStores(r.Context(), email)
	if err != nil {
...
func (h *AuthHandler) CreateManagementAccount(w http.ResponseWriter, r *http.Request) {
	...
	var userID string
	err = pgx.BeginFunc(r.Context(), h.adminDB, func(tx pgx.Tx) error {
		if err := tx.QueryRow(
			r.Context(),
		...
		return observability.InsertAuditLog(r.Context(), tx, observability.AuditEntry{
			...
			ActorID:           actorID,
			ActorType:         actorType,
			ActionType:        "management-account.create",
			ActionDescription: "Created management account",
			TargetEntity:      "management_account",
			TargetID:          &userID,
			Outcome:           "success",
...
func (h *AuthHandler) resolveUserForSignIn(ctx context.Context, email string) (*resolvedUser, error) {
	adminUser, adminFound, err := findAdminUserByEmail(ctx, h.adminDB, email)
	if err != nil {
		return nil, err
	}

	applicantUser, applicantFound, err := findApplicantUserByEmail(ctx, h.applicantDB, email)
	if err != nil {
		return nil, err
	}

	if adminFound && applicantFound {
		return nil, utils.NewHandlerFuncErr(http.StatusConflict, "duplicate account detected for this email")
	}
	if adminFound {
		return adminUser, nil
	}
	if applicantFound {
		return applicantUser, nil
	}

	return nil, utils.NewHandlerFuncErr(http.StatusUnauthorized, "invalid credentials")
}

func (h *AuthHandler) emailExistsAcrossStores(ctx context.Context, email string) (bool, error) {
	adminExists, err := emailExists(ctx, h.adminDB, email)
	if err != nil {
		return false, err
	}
	if adminExists {
	...
	return emailExists(ctx, h.applicantDB, email)
}

func findAdminUserByEmail(ctx context.Context, db *pgxpool.Pool, email string) (*resolvedUser, bool, error) {
	if db == nil {
	...
	if err := row.Scan(
		...
		return nil, false, fmt.Errorf("failed to query admin user: %s", err.Error())
	}

	return &user, true, nil
}

func findApplicantUserByEmail(ctx context.Context, db *pgxpool.Pool, email string) (*resolvedUser, bool, error) {
	if db == nil {
		return nil, false, errors.New("applicant database is not configured")
	}

	row := db.QueryRow(
		ctx,
		`SELECT id, email, password, COALESCE(first_name, ''), COALESCE(last_name, '')
		 FROM users
		 WHERE email = $1`,
		email,
	)

	user := resolvedUser{Role: authz.RoleApplicant}
	if err := row.Scan(
		&user.ID,
		&user.Email,
		&user.Password,
		...
		return nil, false, fmt.Errorf("failed to query applicant user: %s", err.Error())
	}
