...
const adminPasswordResetTokenTTL = 30 * time.Minute

func RequestAdminPasswordReset(ctx context.Context, email string) error {
	...
	token, err := generateAdminResetToken()
	if err != nil {
		return err
	}
	tokenHash := hashAdminResetToken(token)
	expiresAt := time.Now().Add(adminPasswordResetTokenTTL)
...
func ResetAdminPassword(ctx context.Context, token string, newPassword string) error {
	conn, ok := ctx.Value(middlewares.CtxPostgresKey).(*pgxpool.Pool)
	if !ok {
		return errors.New("could not establish connection with the database")
	}
...
func generateAdminResetToken() (string, error) {
	buf := make([]byte, 32)
...
func hashAdminResetToken(token string) string {
