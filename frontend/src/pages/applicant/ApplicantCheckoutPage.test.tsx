...
import { ApplicantCheckoutPage } from "./ApplicantCheckoutPage";
import { routes } from "../../routes/routeConfig";

describe("ApplicantCheckoutPage", () => {
  it("renders an unavailable state instead of a fake payment success flow", () => {
    render(
      <MemoryRouter initialEntries={[routes.applicant.checkout]}>
        <Routes>
          <Route path={routes.applicant.checkout} element={<ApplicantCheckoutPage />} />
        </Routes>
      </MemoryRouter>,
    );
  ...
  it("shows basket preview data as preview-only when route state is provided", () => {
    render(
      <MemoryRouter
        initialEntries={[
          {
            pathname: routes.applicant.checkout,
            state: {
              basketPreview: {
                plan: {
                  id: "plan-1",
